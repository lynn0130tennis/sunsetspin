...
async function runDashboardCompilation() {
    // Safety verification check: verify if we are actually viewing a page containing a user dashboard
    const profileFormCheck = document.getElementById("frm-dashboard-profile");
    const upcomingContainerCheck = document.getElementById("container-upcoming-events");
    
    // Safety exit: if neither element is present, we are on the landing home page. Stop here to prevent app crash.
    if (!profileFormCheck && !upcomingContainerCheck) {
        console.log("Profile framework components absent. Execution skipped safely.");
        return;
    }
...
// Lifecycle Load
window.addEventListener("load", runDashboardCompilation);
